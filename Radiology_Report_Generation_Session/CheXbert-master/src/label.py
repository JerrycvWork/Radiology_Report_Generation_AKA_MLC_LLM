...
def label(checkpoint_path, csv_path):
    ...
    #else:
    checkpoint = torch.load("/home/htihe/PycharmProjects/CIBMProject/CE_Metrics_Calculation/CheXbert-master/CheXbert-master/src/ckpt/chexbert.pth", map_location=torch.device('cuda'))
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(ld)):
            ...
            attn_mask = utils.generate_attention_masks(batch, src_len, device).cuda()
...
def save_preds(y_pred, csv_path, out_path):
    ...
    df.to_csv(os.path.join(out_path,csv_path.split("/")[-1].split('.')[0]+'_labeled_reports.csv'), index=False)
