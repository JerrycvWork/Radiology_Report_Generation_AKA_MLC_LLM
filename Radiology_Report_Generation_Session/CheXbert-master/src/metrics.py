...
def compute_mlc_for_std(gt, pred, label_set):
    res_mlc = {}
    avg_aucroc = 0
    #for i, label in enumerate(label_set):
        #res_mlc['AUCROC_' + label] = roc_auc_score(gt[:, i], pred[:, i])
    ...
    res_mlc['F1_MACRO'] = np.std(f1_score(gt, pred, average=None))
    res_mlc['F1_MICRO'] = np.std(f1_score(gt, pred, average=None))
    res_mlc['RECALL_MACRO'] = np.std(recall_score(gt, pred, average=None))
    res_mlc['RECALL_MICRO'] = np.std(recall_score(gt, pred, average=None))
    res_mlc['PRECISION_MACRO'] = np.std(precision_score(gt, pred, average=None))
    res_mlc['PRECISION_MICRO'] = np.std(precision_score(gt, pred, average=None))
