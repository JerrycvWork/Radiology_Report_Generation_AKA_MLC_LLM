# frequency_analysis_mimic_cxr.py
# Similar to frequency_analysis_iuxray.py, but with defaults for larger dataset and try-except for NaNs.
# Key differences: Add print(i) for progress, handle larger bins default '10,100,1000,10000,100000'.
# Adjust parser default: --bins '10,100,1000,10000,100000'
# Rest is identical; copy and modify defaults accordingly.
import argparse
import collections
import pandas as pd
...
parser.add_argument('--bins', default='10,100,1000,10000,100000', help='Comma-separated bin lowers')
...
# Rest same as iuxray script: frequency, binning, encoding.
...
freq_df.to_csv(f"{args.output_dir}/total_keyword_frequency_mimic.csv", index=False)
...
for idx, low in enumerate(bins[:-1]):
    ...
    case_final.to_csv(f"{args.output_dir}/total_keyword_frequency_mimic_total_{low}.csv", index=False)
...
for split, df in [('train', train_keyword_csv), ('test', test_keyword_csv), ('val', val_keyword_csv)]:
    ...
    case_final.to_csv(f"{args.output_dir}/total_keyword_frequency_mimic_{split}_coding.csv", index=False)